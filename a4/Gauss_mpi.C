         ...
         for(int j=i+1; j<NumSize-Remaindr; j+=nprocs)
            {
              for(int k=1; k<nprocs; k++)
                 {
                   MPI_Isend(matrix[j+k], NumSize+1, MPI_DOUBLE, k, 0, MPI_COMM_WORLD, &request);
                 }
              pivotval = matrix[j][i];
              ...
              for(int k=1; k<nprocs; k++)
                 {
                   MPI_Recv(matrix[j+k], NumSize+1, MPI_DOUBLE, k, 3, MPI_COMM_WORLD, &status);
                 }
            }
         ...
         if (Remaindr>0)
            {
              for(int k=1; k<Remaindr; k++)
                 {
                   MPI_Isend(matrix[i+Quotient*nprocs+k+1], NumSize+1, MPI_DOUBLE, k, 0, MPI_COMM_WORLD, &request);
                 }
              ...
              for(int k=1; k<Remaindr; k++)
                 {
                   MPI_Recv(matrix[i+Quotient*nprocs+k+1], NumSize+1, MPI_DOUBLE, k, 3, MPI_COMM_WORLD, &status);
                 }
            }
       }
}
    ...
    for(int i=0; i<NumSize; i++) 
       { 
         ...
         for(int i=0; i<tasAlloc[proc_id]; i++)
            {
              MPI_Recv(delivMat, NumSize+1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
              ...
              MPI_Ssend(delivMat, NumSize+1, MPI_DOUBLE, 0, 3, MPI_COMM_WORLD);
            }
       }
}
