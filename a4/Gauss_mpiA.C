...
int main(int argc, char *argv[])
{
    ...
    if ((delivMat = (double *) malloc(sizeof(double)*(NumSize+1))) == 0) 
       {
       ...
       {         
        ...
        for(int j=i; j<=NumSize; j++)
         ...
         for(int j=1; j<nprocs; j++)
            {
              MPI_Isend(&matrix[i][i], NumSize+1-i, MPI_DOUBLE, j, 0, MPI_COMM_WORLD, &request);
            }
 
         for(int j=i+1; j<NumSize-Remaindr; j+=nprocs)
            {
              for(int k=1; k<nprocs; k++)
                 {
                   MPI_Isend(&matrix[j+k][i], NumSize+1-i, MPI_DOUBLE, k, 0, MPI_COMM_WORLD, &request);
                 }
              pivotval = matrix[j][i];
              ...
              for(int k=1; k<nprocs; k++)
                 {
                   MPI_Recv(&matrix[j+k][i], NumSize+1-i, MPI_DOUBLE, k, 3, MPI_COMM_WORLD, &status);
                 }
            }
              
         if (Remaindr>0)
            {
              for(int k=1; k<Remaindr; k++)
                 {
                   MPI_Isend(&matrix[i+Quotient*nprocs+k+1][i], NumSize+1-i, MPI_DOUBLE, k, 0, MPI_COMM_WORLD, &request);
                 }
              pivotval = matrix[i+Quotient*nprocs+1][i];
              for(int k=i; k<NumSize+1; k++)
                 {
                   matrix[i+Quotient*nprocs+1][k] -= pivotval*matrix[i][k];
                 }
              for(int k=1; k<Remaindr; k++)
                 {
                   MPI_Recv(&matrix[i+Quotient*nprocs+k+1][i], NumSize+1-i, MPI_DOUBLE, k, 3, MPI_COMM_WORLD, &status);
                 }
            }
       }
}
...
void  sla_fn()
{
    ...
    for(int i=0; i<NumSize; i++) 
       { 
         ...
         MPI_Recv(&pivotRow[elimIdx], NumSize+1-elimIdx, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
         for(int i=0; i<tasAlloc[proc_id]; i++)
            {
              MPI_Recv(&delivMat[elimIdx], NumSize+1-elimIdx, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);

              double pivotval = delivMat[elimIdx];
              for(int k=elimIdx; k<NumSize+1; k++)
                 {
                   delivMat[k] -= pivotval*pivotRow[k];
                 }
              MPI_Ssend(&delivMat[elimIdx], NumSize+1-elimIdx, MPI_DOUBLE, 0, 3, MPI_COMM_WORLD);
            }
       }
}
